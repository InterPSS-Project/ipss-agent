...
Usage:
  python generate_nerc_tpl_report.py <case_display_name> <result_dir>
  python generate_nerc_tpl_report.py 'IEEE 118-Bus Test Case' data/ieee/result
  python generate_nerc_tpl_report.py 'Texas 2000-Bus System' data/psse/Texas2K/result
...
# Default thresholds (used as fallback if config file is missing)
_DEFAULT_CFG = {
    "base_mva": 100000,
    "voltage": {
...
def load_tpl_config():
    """Load TPL report thresholds from config/tpl_report.json, falling back to defaults."""
...
def resolve_case_base(result_dir: str) -> Path:
    """Directory containing *_DF_bus.csv: path relative to wspace/, or name under wspace/result/."""
    p = Path(result_dir)
...
# get display name from cmd line input
case_display_name = sys.argv[1]
...
def load_csv(filename, case_base: Path):
    """Load CSV file from case result directory and return list of dicts."""
...
def parse_network_info(case_base: Path, prefix: str) -> dict:
    """Parse _network_info.txt into a structured dictionary.
    ...
    'loadflow_run' (dict of key→value strings), or None if the file is absent.
    """
...
def _bus_in_service(bus):
    """True if the bus is in service in the AC model (InterPSS `InService` in *_DF_bus.csv).

    Out-of-service buses are excluded from voltage compliance; they often carry **0.0 pu**
    in result exports and must not drive v_min / violation counts.
    """
...
def analyze_voltage_profile(buses, cfg=None):
    ...
    Args:
        buses: List of bus dicts from bus CSV.
        cfg: Optional thresholds dict (uses CFG global if None).

    Returns dict with voltage stats, violation lists, and band distribution.
    """
    if cfg is None:
        cfg = CFG
    ...
    violations_low = []    # Below v_min_limit pu
    violations_high = []   # Above v_max_limit pu
    low_voltage_warn = []  # v_min_limit to v_marg_low (marginal)
    high_voltage_warn = [] # v_marg_high to v_max_limit (marginal high)

    # Voltage band distribution
    bands = {
        "severe_low": [],    # < severe_low
        "violation_low": [], # severe_low to v_min_limit
        "marginal": [],      # v_min_limit to v_marg_low
        "nominal": [],       # v_marg_low to v_marg_high
        "high_ok": [],       # v_marg_high to v_max_limit
        "violation_high": [],# > v_max_limit
    }
    ...
    for bus in in_service:
        ...
        # Floating-point tolerance for compliance boundaries
        fp_tol = cfg["voltage"].get("fp_tol", 0.001)

        # Violation / warning classification (toleranced at compliance limits)
        ...
        # Band distribution (toleranced at compliance boundaries)
...
def analyze_branch_loading(branches, cfg=None):
    """Analyze branch loading (MVA flow vs rating) for Rate A and Rate B.
    ...
    Also computes Rate B (emergency) loading for branches with LimMvaB > 0.
    """
    if cfg is None:
        cfg = CFG
    th = cfg["thermal"]
    ...
    overloaded = []
    moderate_loaded = []
    overloaded_b = []     # Rate B overloads (> overload_pct% of Rate B)
    heavy_loaded_b = []   # Rate B heavily loaded
...
def analyze_generator_q_limits(gens, cfg=None):
    ...
    for gen in gens:
        ...
        # Floating-point tolerance: differences < fp_tol are noise, not real violations
...
def generate_report(case_name, result_dir=None):
    ...
    w(_md_table_row("Field", "Value"))
    w(_md_table_sep(2))
    w(_md_table_row("Generated By", "`generate_nerc_tpl_report.py`"))
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        ...
        print("Usage: python generate_nerc_tpl_report.py <case_name> <result_dir>")
        print("Example: python generate_nerc_tpl_report.py 'IEEE 118-Bus' data/ieee/result")
        print("Example: python generate_nerc_tpl_report.py 'Texas 2K' data/psse/Texas2K/result")
        print("\nBackward-compatible mode also supported:")
        print("  python generate_nerc_tpl_report.py <discovered_alias_or_case_dir>")
        sys.exit(1)
