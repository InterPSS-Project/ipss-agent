...
Markdown reports through small pure-Python scripts. There is no JPype bridge and
no JVM-path configuration.
...
│   └── gen_report.json        # Report band thresholds (used by src/report/)
├── lib/
...
The report generators are pure-Python (standard library only) and read CSV outputs
from a previous simulation run. No `pip` packages are required; run them with any
Python 3 interpreter. The DSH plugin invokes them through `.venv/bin/python`, so
create a bare virtual environment at the project root if the plugin is in use:
...
`src/report/generate_nerc_tpl_report.py` reads CSV outputs and produces a NERC
TPL-001-5 compliance report in Markdown. The CSV prefix is auto-discovered from
the result directory, so you provide a display name and the result directory name:
...
The script writes `NERC_TPL_001_5_Report.md` into the same result directory.

### AC Load Flow report

`src/report/generate_aclf_report.py` reads the `*_DF_{bus,branch,gen,load}.csv`
plus `*_network_info.txt` that the ACLF CLI produces and writes
`AC_Loadflow_Report.md` into the same result directory:
...
Both generators share analysis and Markdown helpers through
`src/report/ipss_report_common.py`, so voltage bands, thermal loading percentages,
and generator Q-limit logic stay aligned. See [GenReport.md](GenReport.md).
...
3. Report generation with `python3 ../src/report/generate_nerc_tpl_report.py ...`
