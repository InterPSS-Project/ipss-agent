...
Run commands from `wspace/` with the venv created at the **project root** (parent of `wspace/`). See [Setup.md](Setup.md) for full layout, JARs, and `config/config.json`.
...
For ACLF, `ipss_cmd.py` picks `aclf_run.json` in this order:
