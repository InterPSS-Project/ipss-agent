...
Reads CSV output from ``../src/main/py/ipss_cmd.py aclf`` (run from ``wspace/``) and produces a neutral
...
Usage (from ``wspace/`` with venv activated):
  python ../src/main/py/report/generate_aclf_report.py <case_display_name> <result_dir> [csv_prefix]
...
use ``../src/main/py/report/generate_nerc_tpl_report.py`` against the same CSV set.
"""
...
def _resolve_prefix(case_base: Path, csv_prefix: str | None = None) -> str:
    ...
    if csv_prefix is not None:
        ...
        if not bus_path.exists():
            raise FileNotFoundError(
                f"No {prefix}_DF_bus.csv under {case_base}. Run `python ../src/main/py/ipss_cmd.py aclf` first."
            )
    ...
    if not bus_files:
        raise FileNotFoundError(
            f"No *_DF_bus.csv files found under {case_base}. Run `python ../src/main/py/ipss_cmd.py aclf` first."
        )
...
def generate_aclf_report(
    ...
    Args:
        ...
        csv_prefix: Optional CSV stem matching ``../src/main/py/ipss_cmd.py`` output (e.g.
    ...
    if ratings_unavailable:
        w(
            ...
            "`python ../src/main/py/ipss_cmd.py aclf` to enable this section."
        )
        w("")
    else:
        w(
    ...
    w(_md_table_row("Field", "Value"))
    w(_md_table_sep(2))
    w(_md_table_row("Generated By", "`src/main/py/report/generate_aclf_report.py`"))
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python ../src/main/py/report/generate_aclf_report.py <case_display_name> <result_dir> [csv_prefix]")
        print(
            "Example: python ../src/main/py/report/generate_aclf_report.py 'IEEE 118-Bus Test Case' data/ieee/Ieee118Bus/result"
        )
        print(
            "Example: python ../src/main/py/report/generate_aclf_report.py 'IEEE 14-Bus System' data/ieee/Ieee14Bus/result ieee14"
        )
        print(
            "Example: python ../src/main/py/report/generate_aclf_report.py 'Texas 2000-Bus System' data/psse/Texas2K/result"
        )
        sys.exit(1)
